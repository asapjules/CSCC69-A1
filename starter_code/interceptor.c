...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//access info in table between locks
	spin_lock(&calltable_lock);
	int monitored = table[reg.ax].monitored;
	spin_unlock(&calltable_lock);

	//if some pids monitored
	if(monitored == 1){
		//access info of pidlist between locks
		spin_lock(&calltable_lock);
		spin_lock(&pidlist_lock);
		monitored = check_pid_monitored(reg.ax, current->pid);
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		//check if syscall is monitored for process' PID
		if(monitored == 1){
			log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
	//if all pids monitored
	else if(monitored == 2){
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}

	//access original system call between locks
	spin_lock(&calltable_lock);
	asmlinkage long orig_call = table[reg.ax].f(reg);
	spin_unlock(&calltable_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//check if pid is valid if cmd is one of the last 2 commands
	...
	//for intercept command
	if(cmd == REQUEST_SYSCALL_INTERCEPT){

		//check intercepted status from table between locks
		spin_lock(&calltable_lock);
		int intercepted = table[syscall].intercepted;
		spin_unlock(&calltable_lock);

		//check if intercepting a syscall that is already intercepted
		if(intercepted == 1){
			return -EBUSY;
		}

		//lock before saving old syscall
		spin_lock(&calltable_lock);
		//save original syscall
		table[syscall].f = sys_call_table[syscall];
		//mark syscall as intercepted
		table[syscall].intercepted = 1;
		//close lock after saving and marking syscall
		spin_unlock(&calltable_lock);
		...
		//revoke rw permission
		set_addr_ro((unsigned long)sys_call_table);
	}
	//for de-intercept command
		...
		//check if syscall has been intercepted
		if(intercepted != 1){
			return -EINVAL;
		}
		//de-intercept the syscall
		set_addr_rw((unsigned long)sys_call_table);
	...
	//for monitor command
		...
		//get monitor status of pid from list between locks
		spin_lock(&calltable_lock);
		spin_lock(&pidlist_lock);
		int monitored = check_pid_monitored(syscall, pid);
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		//check if monitoring pid that is already being monitored
		if(monitored == 1){
			return -EBUSY;
		}
		//check if pid=0
		if(pid == 0){
			//start monitoring for all pids between locks
			spin_lock(&calltable_lock);
			...
			//add to syscall's list of monitored pids between locks
			spin_lock(&calltable_lock);
			spin_lock(&pidlist_lock);
	...
	//for stop monitoring command
	else if(cmd == REQUEST_STOP_MONITORING){
		//get monitor status between locks
		
		spin_lock(&calltable_lock);
		int table_monitored = table[syscall].monitored;
		spin_lock(&pidlist_lock);
		int check_list_monitored = check_pid_monitored(syscall, pid);
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);

		//if syscall not monitored
		if(table_monitored != 2 && check_list_monitored != 1){
			return -EINVAL;
		}

		//get intercept status between locks
		spin_lock(&calltable_lock);
		int intercepted = table[syscall].intercepted;
		spin_unlock(&calltable_lock);

		//if syscall not intercepted
		if(intercepted != 1){
			return -EINVAL;
		}
		//check if pid=0
		if(pid == 0){
			//stop monitoring for all pids between locks
			spin_lock(&calltable_lock);
			...
			//remove from syscall's list of monitored pids between locks
			spin_lock(&calltable_lock);
			spin_lock(&pidlist_lock);
...
static int init_function(void) {
	...
	//set rw permission for syscall table
	set_addr_rw((unsigned long) sys_call_table);
	//initialize interceptor info table between locks
	spin_lock(&calltable_lock);
	spin_lock(&pidlist_lock);
	int i = 0;
	...
	//hijack the exit_group system call and save the original
	orig_exit_group = sys_call_table[__NR_exit_group];
	sys_call_table[__NR_exit_group] = my_exit_group;
	
	//set ro permission for syscall table
	set_addr_ro((unsigned long) sys_call_table);
